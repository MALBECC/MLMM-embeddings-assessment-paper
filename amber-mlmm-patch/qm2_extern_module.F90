...
#endif
  use qm2_extern_mlmm_module   , only: get_mlmm_forces
  implicit none
...
#include "../include/md.h" /* We are only using this for values of nstlim and maxcyc */
    ...
    if(first_call) then
      ...
      if (extern_program == 'ml') call check_mlmm_embedding(nclatoms, qmmm_nml%qmmm_int)
...
#endif
    case('ml')
      call get_mlmm_forces( nstep, ntpr, nqmatoms, qmcoords,&
      nclatoms, clcoords, escf, dxyzqm, dxyzcl)
    case default
...
#  endif
  ...
  ! Check that ani is running with qmmm_int=0,2 or 5
  subroutine check_mlmm_embedding(nclatoms, qmmm_int)
    ...
    if (nclatoms > 0) then
       if ( .not. ((qmmm_int == 0) .or. (qmmm_int == 2) .or. (qmmm_int == 5))) then
             call sander_bomb("qm2_extern_get_qm_forces","nquant /= natom", &
                  "Trorchani based ML/MM calculations only support qmmm_int = 0, 2 or 5")
       end if
    end if

  end subroutine check_mlmm_embedding
